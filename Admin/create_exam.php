...
$courses = $conn->query("SELECT id, title FROM courses WHERE created_by = $admin_id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $stmt = $conn->prepare("INSERT INTO exams (course_id, title, type, duration) VALUES (?, ?, ?, ?)");
  $stmt->bind_param("issi", $course_id, $title, $type, $duration);

  if ($stmt->execute()) {
    echo "<script>alert('✅ Exam created successfully!'); window.location.href='add_question.php?exam_id=" . $stmt->insert_id . "';</script>";
  } else {
...
<head>
  <meta charset="UTF-8" />
  <title>Create Exam - Cyber SecuraX</title>
...
<div class="sidebar">
    ...
    <nav>
      ...
      <a href="create_exam.php"class="active"><i class="fas fa-question-circle"></i> <span class="d-none d-md-inline">Create Exam</span></a>
...
<div class="main-content">
  <div class="container">
    <h2 class="text-center text-success mb-4">🧪 Create New Exam</h2>
    <form method="POST">
      <div class="mb-3">
      ...
      <button type="submit" class="btn w-100">➕ Create Exam</button>
    </form>
  </div>
</div>
